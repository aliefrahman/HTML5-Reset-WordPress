 ...
 */
	...
	add_action( 'after_setup_theme', 'ultrablog_setup' );

	// Scripts & Styles (based on twentythirteen: http://make.wordpress.org/core/tag/twentythirteen/)
	function ultrablog_scripts_styles() {
		global $wp_styles;
...
//		$wp_styles->add_data( 'html5reset-ie', 'conditional', 'lt IE 9' );
	...
	add_action( 'wp_enqueue_scripts', 'ultrablog_scripts_styles' );

	// WP Title (based on twentythirteen: http://make.wordpress.org/core/tag/twentythirteen/)
	function ultrablog_wp_title( $title, $sep ) {
		global $paged, $page;
...
//		 Add a page number if necessary.
		if ( $paged >= 2 || $page >= 2 )
			$title = "$title $sep " . sprintf( __( 'Page %s', 'ultrablog' ), max( $paged, $page ) );

		return $title;
	}
	add_filter( 'wp_title', 'ultrablog_wp_title', 10, 2 );
...
//OLD STUFF BELOW
	...
	if ( function_exists('register_sidebar' )) {
		function ultrablog_widgets_init() {
		...
		add_action( 'widgets_init', 'ultrablog_widgets_init' );
	}
